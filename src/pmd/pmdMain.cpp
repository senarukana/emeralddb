...
static int pmdResolveArguments(int argc, char **argv) {
    ...
    if (rc) {
        PD_LOG(PDERROR, "Failed to init krcv, rc = %d", rc);
        goto error;
    }
done:
    return rc;
...
#define PMD_MAX_SGINALS 64
static void pmdSignalHandler(int sigNum) {
    if (sigNum > 0 && sigNum <= PMD_MAX_SGINALS) {
...
static void pmdSetupSignalHandler() {
    struct sigaction newact;
    sigemptyset(&newact.sa_mask);

    newact.sa_flags = 0;
    newact.sa_handler = (__sighandler_t)pmdSignalHandler;
    for (int i = 0; i <PMD_MAX_SGINALS; i++) {
